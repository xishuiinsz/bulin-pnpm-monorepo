...
import ExpandCollapseRows from "@/components/table/expand-collapse-rows/ExpandCollapseRows.vue";
import { useExpandCollapseRows } from "@/components/table/utils";
...
const { tableData, expandedRowKeys, handleExpand, handleCollapse } = useExpandCollapseRows({
  refFlag: false,
  childKey: childrenField,
  defaultExpandedKeys: [],
  getRowKey: getRowKey,
});

tableData.push(...structuredClone(tableList));
...
function getExpandedRows(treeData: User[], expandedKeys: string[]): User[] {
  ...
  function dfs(node: User, depth: number, shouldShow: boolean) {
    ...
    if (node.childrenList) {
      node.childrenList.forEach((child) => dfs(child, depth + 1, childrenShouldShow));
    }
  }
        ...
        <el-table :span-method="spanMethod" class="expand-collapse-table mt-4" :data="tableData" style="width: 100%"
          ...
          <el-table-column prop="surname" label="姓氏">
            <template #default="scope">
              <ExpandCollapseRows :getRowKey="getRowKey" :expandedRowKeys="expandedRowKeys" v-bind="scope"
                :childKey="childrenField" :handleExpand="handleExpand" :handleCollapse="handleCollapse" />
            </template>
          </el-table-column>
